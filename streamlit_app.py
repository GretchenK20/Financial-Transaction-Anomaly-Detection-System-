...
Streamlit app — connects to FastAPI endpoint for live scoring.
"""
...
with st.sidebar:
    ...
    try:
        champ = requests.get(f"{API_BASE}/champion", timeout=5).json()
...
with tab1:
    ...
    if score_btn:
        with st.spinner("Scoring transaction..."):
            try:
                r = requests.get(f"{api_url}/transaction/{int(transaction_id)}", timeout=30)
...
with tab2:
    ...
    sample_ids = [0, 1, 2, 3, 243393, 243547]
    ...
    if batch_btn:
        ...
        with st.spinner(f"Scoring {len(ids)} transactions..."):
            try:
                ...
                errors = []
                for tid in ids:
                    r = requests.get(f"{api_url}/transaction/{tid}", timeout=30)
                    if r.status_code == 200:
                        results.append(r.json())
                    else:
                        errors.append({"transaction_id": tid, "error": r.text})
                ...
                if errors:
                    st.warning(f"{len(errors)} transaction(s) failed to score")
...
with tab3:
    ...
    if ANOMALY_SCORES_PATH.exists():
        ...
        st.warning("No anomaly_scores.parquet found — run model training first.")
